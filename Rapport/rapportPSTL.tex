...
\subsection{OCaml-Java : compilation de code OCaml vers du bytecode Java}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%  2  REALISATION   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Portage d'O'Jacaré pour OCaml-Java : \emph{O'Jacaré 2}}
...
Le module Cidl définit le nouvel AST nommé CAST\footnote{Pour \emph{Checked AST}} allant être manipulé dans les passes de
génération de code.
...
\begin{tabular}{|c|l|l|}
  \hline
  élément & descriptions supplémentaires & modifications à faire pour OCaml-Java\\
  \hline
  ...
  \hline
  Cast JNI (up et down) & utile pour le cast utilisateur & inutile : une fonction de OCaml-Java (Java.cast) le fait déjà\\
  \hline
  Fonction d'allocation & alloue la référence Java du côté OCaml & inutile : OCaml-Java n'utilise que le runtime Java \\
  \hline
  Capsule & vérifications avant l'appel vers Java & inutile : les vérifications se font lors de la compilation\\
  & arguements des méthodes & conversion des types \\
  \hline
