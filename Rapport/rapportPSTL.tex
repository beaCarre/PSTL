...
Deux travaux ont déjà été réalisées pour l’interopérabilité entre
OCaml et Java à travers leur modèle object respectif :
\begin{itemize}
\item \emph{O’Jacaré} conserve les runtimes des deux langages (GC,
...
Le code Java généré pour le callback vont permettre avec le même
...
\camljava gère l'interfacage entre OCaml et Java via C, comme décrit
dans la figure 3.

Un extrait d'utilisation de cette bibliothèque est :
\begin{OCamlEx}
...
Mais cette gestion avec deux Garbage Collector et deux ensembles de racine pour la mémoire reste incertaine, dans la mesure où 
...
\newpage
Voici un exemple d'utilisation du module Java d'OCaml, voué à être compilé avec \emph{OCaml-Java} :
\begin{OCamlEx}
...
Après cela, il a fallut partir des fichiers générés d'O'Jacaré, et faire une ébauche à la main (en supprimant l'inutile et en adaptant les appels vers Java).
...
\begin{tabular}{|c|l|l|l|}
 \hline
TYPE IDL &type Java & type OCaml pour \emph{OCaml-Java} & type OCaml \\
...
Le type "top" manipulé sera le type d'une instance de Ocaml-Java de la classe Objet Java :
\begin{OCamlEx}
type top = java'lang'Object java_instance;;
\end{OCamlEx}

Une exception a été ajoutée, TODO :
\begin{OCamlEx}
