...
let make_dyn clazz java_obj ~callback m_list =
  let make m =
	...
	if abstract && callback then
	  ...
	  let call_method = MlType.get_call_method ~virtual_call:(not callback) rtyp in 
	  ...
	  let body = 
	    if not callback then
	      <:expr< $id:call_method$ $lid:java_obj$ $lid:id$ [| $list:jargs$ |] >> 
	    else
	      <:expr< $id:call_method$ $lid:java_obj$ $lid:clazz$ $lid:id$ [| $list:jargs$ |] >> in
	  ...
	  let err = "Unknown method from IDL in class \\\""^
	  ...
	  Some (id, safe_id_expr),
	  <:class_str_item< method $lid:ml_name$ = $body$ >>

    | Cset typ ->   
	let id = fid_prefix^java_name
	and call_method_name = <:ident< Jni. $lid:"set_"^(MlType.string_of_type typ)^"_field"$ >>  
	and narg = "_p" in
	...
	and call_method_name = <:ident< Jni. $lid:"get_"^(MlType.string_of_type typ)^"_field"$ >>  in
...
(** Génère les méthodes appelés par java *)
...
let make_static cl_list =
  ...
  let make_cl cl =
    ...
    let make m = 
	  ...
	  let call_method_name = "Jni.set_static_"^(MlType.string_of_type typ)^"_field"  
	  and narg = "_p" in
	  let call = <:expr< $anti:call_method_name$ $lid:clazz$ $lid:id$ $lid:narg$ >> in
